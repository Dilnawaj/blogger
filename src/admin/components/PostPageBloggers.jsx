...
function PostPageBloggers() {
  ...
  const handleGoBack = () => {
    ...
    console.log("Current URL Bhai", `/home?pageNumber=${currentPageNumber}&sortBy=${currentSortBy}&keyword=${currentKeyword}`);
  
    // Set dynamic values in local storage
    ...
    // Navigate using the dynamic values
    navigate(`/home/admin?pageNumber=${currentPageNumber}&sortBy=${currentSortBy}&keyword=${currentKeyword}`);
  };
...
console.log(data);
  ...
  useEffect(() => {
    const currentUrl = window.location.pathname;
    console.log("Current URLLLLLLLLLLLLLLXXXXXXXXXX", keyword);
    loadPost(postId)
      .then((data) => {
        console.log(data);
        //user?.id
...
export default PostPageBloggers;
